...
(function($) {
    ...
    jQuery(document).ready(function($) {
        ...
        $('.on_image').on("click", function() {
            $('.ed_event_wrapper_item_img').html('<img src="https://dummyimage.com/263x263/000/fff" alt="item1" class="img-responsive">');
            $('.on_image').hide();
            $('.on_map').show();
        });
...
// Click to remove
        $(document).ready(function() {
            ...
            // Attach the click event handler to the element
            $("#myButton").on("click", handleClick);
      
            // Remove the click event handler when the element is clicked
        ...
        if (widthh <= 1199) {
            $(window).scroll(function() {
                var window_top = $(window).scrollTop() + 1;
                if (window_top > 500) {
                    $('.ed_header_bottom').addClass('menu_fixed');
                } else {
                    $('.ed_header_bottom').removeClass('menu_fixed');
                }
            });
        }
